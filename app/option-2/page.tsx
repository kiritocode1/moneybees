...
function Tag({ children, inverse = false }: { children: React.ReactNode; inverse?: boolean }) {
  return (
    <span className={`inline-flex min-h-[29px] items-center rounded-[3px] px-4 text-[11px] font-medium uppercase tracking-[.12em] ${inverse ? "bg-white text-[#272727]" : "bg-[#272727] text-white"}`}>
      {children}
    </span>
  );
}
...
function PrimaryButton({ children, href = "#contact", ghost = false }: { children: React.ReactNode; href?: string; ghost?: boolean }) {
  return (
    ...
    >
      <span aria-hidden className={`option-two-button-swipe ${ghost ? "bg-white" : "bg-[#d78138]"}`} />
...
function Header() {
  ...
  return (
    ...
    >
      <div aria-hidden className="absolute inset-0 rounded-[4px] bg-[rgba(255,255,255,.55)] backdrop-blur-[15px]" />
              ...
              >
                ...
                >
                  ...
                  <span aria-hidden className="grid h-4 w-4 place-items-center">
                    <span className={`-mt-1 h-[7px] w-[7px] rotate-45 border-b-2 border-r-2 border-[#9ea0a4] transition-transform duration-300 ${active ? "rotate-[225deg]" : ""}`} />
                  </span>
                </button>
                <div
                  aria-hidden={!active}
                    ...
                    <div className="flex h-[383px] w-[42rem] shrink-0 flex-col gap-4 py-4 pl-6">
                      ...
                      <div className="flex h-[215px] w-[648px] gap-3">
                        ...
                        <a key={image} href={item.hrefs[index]} className="group/card flex h-[215px] w-52 flex-col items-start gap-[12.8px]">
                          ...
                          <span className="flex h-[26px] items-center rounded-[4px] bg-[#f49340] px-[9.6px] py-1 text-xs font-medium uppercase leading-[18px] tracking-[1.2px] text-[#272727]">{item.cardPrefix} #{index + 1}</span>
                        </a>
                      ))}
                      </div>
                    </div>
      ...
      <div className={`absolute inset-x-0 top-[68px] h-[497px] overflow-hidden bg-[#f7f7f8] px-2 py-4 text-[#272727] transition-[opacity,transform,visibility] duration-400 min-[992px]:hidden ${open ? "visible translate-y-0 opacity-100" : "pointer-events-none invisible -translate-y-3 opacity-0"}`}>
        <nav aria-label="Mobile navigation">
          ...
          <div className="flex flex-col gap-4">
            ...
            <a href="#contact" onClick={() => setOpen(false)} className="flex h-12 items-center justify-center rounded-[4px] bg-[#f49340] px-6 text-base text-[#272727]">Get started</a>
          </div>
        </nav>
      </div>
    </header>
...
export default function Option2() {
  ...
  return (
    <main ref={root} id="top" className="option-two overflow-clip bg-[#ffffff] text-[#272727]">
          ...
          <div className="absolute inset-x-[4.5%] bottom-[8%] z-10 grid items-end gap-12 text-[#ffffff] md:grid-cols-[2.05fr_.95fr]">
            <div>
              <div data-hero-tag className="mb-5 md:hidden">
                <span className="inline-flex items-center gap-3 rounded-[4px] bg-white/[.06] px-3 py-2 text-[12px] uppercase tracking-[.18em]"><i className="h-1 w-1 rounded-full bg-[#f49340]" /> Built for the long term <i className="h-1 w-1 rounded-full bg-[#f49340]" /></span>
              </div>
              <div className="relative max-w-[850px]">
                ...
                <span data-hero-tag className="absolute bottom-3 left-[345px] hidden items-center gap-3 rounded-[4px] bg-white/[.06] px-4 py-3 text-[11px] uppercase tracking-[.18em] md:inline-flex"><i className="h-1 w-1 rounded-full bg-[#f49340]" /> Built for the long term <i className="h-1 w-1 rounded-full bg-[#f49340]" /></span>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-[1232px]">
          ...
          <div data-stagger className="mt-16 grid gap-4 md:mt-20 md:grid-cols-3">
            <article className="relative flex min-h-[430px] flex-col justify-between rounded-[5px] bg-[#edeeef] p-6 md:min-h-[500px]">
              <i aria-hidden className="absolute right-2 top-2 h-3 w-3 rounded-[2px] bg-[#f49340]" />
            ...
            <article className="relative min-h-[500px] overflow-hidden rounded-[5px] bg-[#edeeef]">
              ...
              <span aria-hidden className="absolute bottom-2 left-2 flex gap-1">{[0,1,2,3,4].map((n)=><i key={n} className="h-3 w-3 rounded-[2px] bg-[#edeeef]" />)}</span>
            </article>
            <article className="relative flex min-h-[430px] flex-col justify-between rounded-[5px] bg-[#272727] p-6 text-[#ffffff] md:min-h-[500px]">
              <i aria-hidden className="absolute right-2 top-2 h-3 w-3 rounded-[2px] bg-[#edeeef]" />
      ...
      <section id="insights" className="overflow-hidden bg-[#edeeef] px-4 py-20 md:px-10 md:py-32">
        <div className="mx-auto max-w-[1360px]">
          ...
          <div ref={insightTrack} data-reveal className="option-two-track mt-16 flex snap-x gap-6 overflow-x-auto pb-2">
            {insights.map((item) => (
              <article key={item.title} className="option-two-insight-card flex min-h-[384px] w-[min(86vw,512px)] shrink-0 snap-start flex-col justify-between rounded-[4px] border-t-[7px] border-[#f49340] bg-white p-10">
                ...
                <div className="flex items-center gap-4 border-t border-black/10 pt-6">
                  ...
                  <div><strong className="block text-[15px] font-medium">{item.author}</strong><span className="text-[13px] text-[#5b5d61]">{item.role}</span></div>
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="flex flex-col justify-center px-6 py-20 md:px-[9%] lg:py-16">
            ...
            <div data-stagger className="mt-12 space-y-2">
              {approach.map((item,index)=>(
                <button key={item.title} type="button" onClick={()=>setActiveApproach(index)} className={`relative grid w-full grid-cols-[48px_1fr] gap-x-4 overflow-hidden rounded-[3px] p-5 text-left transition-[background-color] duration-200 ${activeApproach===index?"bg-white/[.09]":"hover:bg-white/[.035]"}`}>
                  ...
                  <span aria-hidden className="absolute inset-x-0 bottom-0 h-px bg-white/10">
                    {activeApproach === index && <span key={`progress-${activeApproach}`} className="option-two-tab-progress block h-full bg-[#f49340]" />}
                  </span>
                </button>
              ))}
            </div>
      ...
      <section id="stewardship" className="overflow-hidden px-4 pt-24 md:px-10 md:pt-32">
        <div className="mx-auto max-w-[1360px]">
          ...
          <p data-reveal className="ml-auto mt-16 max-w-[560px] text-[15px] leading-6 text-[#5b5d61]">Moneybee is built for investors who value a comprehensible process over market predictions. We research deeply, communicate plainly, and treat every mandate as capital held in trust.</p>
          <div data-stagger className="mt-24 grid gap-4 md:grid-cols-[1.25fr_.85fr]">
            ...
            <div className="grid gap-4 md:grid-rows-[1.12fr_.88fr]">
              ...
              <article className="relative min-h-[300px] overflow-hidden rounded-[4px] bg-[#272727] p-7 text-white"><Image src="/option-2/hero-poster.jpg" alt="" fill sizes="40vw" className="object-cover opacity-40" /><div className="absolute inset-0 bg-[#272727]/45" /><h3 className="relative max-w-[12ch] text-[34px] font-light leading-[1.18]">A process made to be understood.</h3><p className="absolute bottom-7 left-7 max-w-[34ch] text-sm leading-5">Every position should have a clear thesis, a price, and a reason to remain in the portfolio.</p></article>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="flex min-h-[1100px] p-2 md:min-h-svh">
        <div className="relative min-h-[1084px] w-full overflow-hidden rounded-[4px] text-white md:min-h-[calc(100svh-16px)]">
          ...
          <article data-reveal className="absolute left-[4%] top-[37%] flex min-h-[380px] w-[92%] flex-col justify-between rounded-[4px] bg-[linear-gradient(150deg,rgba(158,160,164,.9),rgba(39,39,39,.9))] p-7 backdrop-blur-sm md:bottom-[7%] md:left-auto md:right-[4%] md:top-auto md:min-h-[430px] md:w-[min(90%,350px)]">
            <i aria-hidden className="absolute right-3 top-3 h-3 w-3 rounded-[2px] bg-[#f49340]" />
      ...
      <section id="testimonials" className="overflow-hidden bg-[#edeeef] px-4 py-20 md:px-10 md:py-32">
        <div className="mx-auto max-w-[1360px]">
          ...
          <div ref={testimonialTrack} data-reveal className="option-two-track mt-16 flex snap-x gap-6 overflow-x-auto pb-2">
            {testimonials.map((item)=>(
              <article key={item.quote} className={`flex min-h-[432px] w-[min(90vw,352px)] shrink-0 snap-start flex-col justify-between rounded-[5px] p-6 ${item.tone==='green'?"bg-[#272727] text-white":"bg-[#ffffff]"}`}>
                <div><span className={`text-4xl font-medium ${item.tone==='green'?"text-white":"text-[#f49340]"}`}>“</span><h3 className="mt-4 text-[24px] font-light leading-[1.45]">{item.quote}</h3><p className="mt-4 text-[13px] leading-5 opacity-80">{item.copy}</p></div>
          ...
          <div data-stagger className="mt-16 space-y-2">
            {faqs.map(([question,answer],index)=>(
              <article key={question} className="rounded-[8px] bg-[#edeeef]">
                ...
                <div className={`grid transition-[grid-template-rows] duration-500 ${openFaq===index?"grid-rows-[1fr]":"grid-rows-[0fr]"}`}><div className="overflow-hidden"><p className="max-w-[850px] px-6 pb-7 text-[15px] leading-6 text-[#5b5d61]">{answer}</p></div></div>
              </article>
            ))}
          </div>
        </div>
      ...
      <footer className="bg-[#1c1c1c] text-[#ffffff]">
        <div className="mx-auto flex max-w-[1296px] flex-col gap-28 px-6 pb-12 pt-24 md:px-0">
          <div data-reveal className="grid gap-16 md:grid-cols-[1.15fr_.9fr]">
            <div><Mark /><p className="mt-10 max-w-[380px] text-[15px] leading-6">Long-term investing perspectives from the Moneybee desk, shared with clarity.</p><form className="mt-6 flex max-w-[480px]" onSubmit={(event)=>event.preventDefault()}><input type="email" aria-label="Email address" placeholder="Enter your email" className="h-12 min-w-0 flex-1 bg-white/20 px-4 text-sm outline-none placeholder:text-white/65 focus:bg-white/25"/><button type="submit" aria-label="Subscribe" className="h-12 w-14 bg-[#f49340] text-[#272727]">›</button></form><p className="mt-5 text-[11px] uppercase tracking-[.12em] text-white/45">Your information is never disclosed to third parties.</p></div>
          ...
          <div data-reveal className="flex items-end justify-between"><p className="text-[clamp(3.4rem,9vw,8rem)] font-light leading-none tracking-[-.05em]">MONEYBEE®</p><a href="#top" aria-label="Back to top" className="option-two-icon-roll-trigger flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#f49340] text-[#272727]"><span className="option-two-icon-roll"><span className="option-two-icon-copy is-base"><ArrowIcon direction="up" /></span><span className="option-two-icon-copy is-hover"><ArrowIcon direction="up" /></span></span></a></div>
        </div>
